...
def _make_ctx(user_id, is_superadmin=False, memberships_by_org=None):
    """Helper to create a CurrentUserContext for testing."""
...
def test_can_manage_org_superadmin(user_and_orgs):
    ...
    # Rebuild as superadmin since the dataclass is frozen
    super_ctx = _make_ctx(ctx.id, is_superadmin=True, memberships_by_org=ctx.memberships_by_org)
    assert can_manage_org(org1, super_ctx) is True
...
def test_can_write_org_viewer_denied(user_and_orgs):
    user_id, org1, org2, _ = user_and_orgs
    # Build a new context with org1 set to viewer
    viewer_ctx = _make_ctx(
        user_id,
        memberships_by_org={
            str(org1): {"role": "viewer", "can_read": True, "can_write": False},
            str(org2): {"role": "editor", "can_read": True, "can_write": True},
        },
    )
    class R: visibility_scope="organization"; organization_id=org1
    assert can_write(R(), viewer_ctx) is False


def test_can_move_scope_to_org_requires_manage(user_and_orgs):
    user_id, org1, org2, ctx = user_and_orgs
    ...
    # Rebuild with org1 downgraded to editor -> no manage
    editor_ctx = _make_ctx(
        user_id,
        memberships_by_org={
            str(org1): {"role": "editor", "can_read": True, "can_write": True},
            str(org2): {"role": "editor", "can_read": True, "can_write": True},
        },
    )
    assert can_move_scope(R(), "organization", org1, editor_ctx) is False
...
def test_superadmin_overrides(user_and_orgs):
    user_id, org1, _, ctx = user_and_orgs
    super_ctx = _make_ctx(user_id, is_superadmin=True, memberships_by_org=ctx.memberships_by_org)
    class R: visibility_scope="organization"; organization_id=org1; owner_user_id=uuid.uuid4()
    assert can_write(R(), super_ctx) is True
    assert can_move_scope(R(), "organization", org1, super_ctx) is True
...
def test_get_org_membership_none_cases():
    ...
    # No org_id
    assert get_org_membership(None, _make_ctx(uuid.uuid4())) is None

    # User with no memberships
    user = _make_ctx(uuid.uuid4())
...
def test_get_org_membership_from_list():
    ...
    mem = {"organization_id": org_id, "role": "admin", "can_read": True, "can_write": True}
...
def test_get_org_membership_malformed_data():
    """Test get_org_membership with malformed/empty data."""
    user = _make_ctx(uuid.uuid4())
    assert get_org_membership(str(uuid.uuid4()), user) is None
...
def test_is_member_of_org_superadmin():
    ...
    user = _make_ctx(str(uuid.uuid4()), is_superadmin=True)
...
def test_can_move_scope_to_public():
    ...
    # But superadmin can
    super_user = _make_ctx(user_id, is_superadmin=True, memberships_by_org=user.memberships_by_org)
    assert can_move_scope(R(), "public", None, super_user) is True
