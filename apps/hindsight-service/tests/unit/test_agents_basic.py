...
def test_agents_crud_organization_scope(db_session):
    ...
    r_create = client.post("/agents/", json=payload, headers=_h(owner_email))
    assert r_create.status_code == 201, r_create.text
    ag = r_create.json()
    ag_id = ag["agent_id"]

    # List agents in org scope
    r_list = client.get(f"/agents/?scope=organization&organization_id={org_id}", headers=_h(owner_email))
    assert r_list.status_code == 200
    assert any(a.get("agent_id") == ag_id for a in r_list.json())

    # Get agent by id
    r_get = client.get(f"/agents/{ag_id}", headers=_h(owner_email))
    ...
    # Delete agent
    r_del = client.delete(f"/agents/{ag_id}", headers=_h(owner_email))
