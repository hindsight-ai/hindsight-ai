"""
Organizations API (fixed variant).

Alternate implementation preserving earlier behavior; prefer `orgs.py` for
current endpoints and logic.
"""
...
from core.utils.role_permissions import get_role_permissions, validate_role
...
def add_member(
    ...
    # Allow explicit permission overrides, but default to role-based permissions
...
def update_member(
    ...
    if "role" in payload and payload["role"]:
        ...
        # Update permissions based on new role if not explicitly overridden
        role_permissions = get_role_permissions(new_role)
        if "can_write" not in payload:
            m.can_write = role_permissions["can_write"]
        if "can_read" not in payload:
            m.can_read = role_permissions["can_read"]
