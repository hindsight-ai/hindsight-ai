...
Usage:
    python consolidation_worker.py
...
def analyze_duplicates_with_llm(memory_blocks: List[Dict[str, Any]], llm_api_key: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for group in duplicate_groups:
            ...
            if not blocks_data:
                logger.warning(f"No blocks data found for group {group_id}, skipping")
                enriched_groups.append(group)
                continue
            
            prompt = """You are an AI assistant tasked with generating consolidated content for a group of duplicate memory blocks. Analyze the content, lessons learned, and keywords of the provided memory blocks. Provide a consolidated version of the content, lessons learned, and keywords for this group. The consolidated content and lessons learned must be of higher quality, increased density of information, and ideally reduced overall size compared to the sum of the original blocks. Do not simply concatenate or expand the original content; aim for concise, high-value summaries. IMPORTANT CONSTRAINT: The suggested consolidation content and lessons learned MUST NOT EXCEED the maximal length of any individual candidate memory block in the group, at all cost! Output the result in JSON format with the structure as shown below:
{
  "suggested_content": "Consolidated content here",
  "suggested_lessons_learned": "Consolidated lessons here",
