...
class UserNotificationPreference(Base):
    ...
    event_type = Column(String(50), nullable=False)  # 'org_invitation', 'org_membership_added', etc.
...
class Notification(Base):
    ...
    action_url = Column(String(500), nullable=True)  # Optional URL for action buttons
    action_text = Column(String(100), nullable=True)  # Text for action button
    metadata_json = Column('metadata', JSONB, nullable=True)  # Additional event-specific data
    is_read = Column(Boolean, nullable=False, default=False)
    read_at = Column(DateTime(timezone=True), nullable=True)
    created_at = Column(DateTime(timezone=True), default=now_utc, nullable=False)
    # Auto-cleanup: notifications older than 30 days can be pruned
...
class EmailNotificationLog(Base):
    ...
    user_id = Column(UUID(as_uuid=True), ForeignKey('users.id', ondelete='CASCADE'), nullable=False)
    email_address = Column(String(320), nullable=False)  # RFC 5321 max email length
    event_type = Column(String(50), nullable=False)
    subject = Column(String(200), nullable=False)
    status = Column(String(20), nullable=False, default='pending')  # 'pending', 'sent', 'failed', 'bounced'
    provider_message_id = Column(String(255), nullable=True)  # Email service provider message ID
