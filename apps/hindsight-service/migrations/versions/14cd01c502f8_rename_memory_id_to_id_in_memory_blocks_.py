...
def upgrade() -> None:
    ...
    op.alter_column('memory_blocks', 'memory_id', new_column_name='id',
                    existing_type=postgresql.UUID(as_uuid=True),
                    type_=postgresql.UUID(as_uuid=True), # Added type_ parameter
                    nullable=False,
                    existing_nullable=False,
                    postgresql_using='memory_id::uuid') # Ensure type cast if needed

    # Recreate foreign key constraints referencing the new 'id' column
    op.create_foreign_key(op.f('feedback_logs_memory_id_fkey'), 'feedback_logs', 'memory_blocks', ['memory_id'], ['id'])
    op.create_foreign_key(op.f('memory_block_keywords_memory_id_fkey'), 'memory_block_keywords', 'memory_blocks', ['memory_id'], ['id'])
