...
const PruningSuggestions = () => {
  ...
  const [pruningParams, setPruningParams] = useState({
    batch_size: 50,
    target_count: 100,
    max_iterations: 10
  });
  ...
  const fetchPruningSuggestions = useCallback(async () => {
    ...
    try {
      ...
      setError('Failed to fetch pruning suggestions: ' + err.message);
  ...
  const handleConfirmPruning = async () => {
    ...
    try {
      ...
      setError('Failed to confirm pruning: ' + err.message);
                ...
                <div key={block.memory_block_id} className="data-table-row">
                  ...
                  <div className="data-cell">
                    <span className={`score-badge ${(() => {
                      ...
                      {(() => {
                        const score = block.pruning_score;
                        return (score !== undefined && score !== null && score !== '') ? parseFloat(score).toFixed(2) : 'N/A';
                      })()}
                    </span>
                  </div>
