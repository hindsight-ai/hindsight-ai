...
const MemoryOptimizationCenter = () => {
  ...
  const [processMeta, setProcessMeta] = useState({ processed: 0, total: 0, startedAt: null, etaMs: 0 });
  const abortCtrlRef = useRef(null);
  
  // Debug panel state
  const [showDebugPanel, setShowDebugPanel] = useState(false);
  const [debugBlockId, setDebugBlockId] = useState('');
  const [debugLoading, setDebugLoading] = useState(false);
  const [debugBlockResult, setDebugBlockResult] = useState(null);
  ...
  const handleViewBlockDetails = async (blockId) => {
    try {
      ...
      try {
        const blockData = await memoryService.getMemoryBlockById(blockId);
  ...
  const handleExecuteSuggestion = async (suggestion) => {
  ...
  const handleKeywordSuggestionsApplied = (result) => {
    // Provide detailed feedback to the user
    const results = result?.results || [];
    const totalAdded = results.reduce((sum, r) => sum + (r.added_keywords?.length || 0), 0);
    const totalSkipped = results.reduce((sum, r) => sum + (r.skipped_keywords?.length || 0), 0);
  ...
  const handleCompactionConfirmed = async (settings) => {
  ...
  const testBlockId = async (blockId) => {
  ...
  const DetailModal = ({ suggestion, isOpen, onClose }) => {
              ...
              <div>
                ...
                >
                  ...
                  {availableAgents.map((agent, index) => (
                    <option key={agent.agent_id || agent.id || `agent-${index}`} value={agent.agent_id || agent.id || ''}>
                  ...
                  {debugBlockResult.success ? (
                    <div>
                      ...
                      <div className="text-sm space-y-1">
                        ...
                        <div><strong>Created:</strong> {debugBlockResult.data.created_at}</div>
                        <div><strong>Archived:</strong> {debugBlockResult.data.archived ? 'Yes' : 'No'}</div>
