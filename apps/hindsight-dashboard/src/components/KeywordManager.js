...
const KeywordManager = () => {
  ...
  const [associatedMemoryBlocks, setAssociatedMemoryBlocks] = useState([]);
  const [loadingAssociations, setLoadingAssociations] = useState(false);
  const [keywordUsageCounts, setKeywordUsageCounts] = useState({});
  ...
  const handleAddKeyword = async () => {
    ...
    try {
      ...
      notificationService.showError('Failed to add keyword: ' + err.message);
    }
  };
  ...
  const fetchKeywordUsageCounts = async () => {
    const counts = {};
  ...
  const handleViewAssociations = async (keyword) => {
                      ...
                      <div className="flex justify-between items-start mb-3">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-xs text-gray-500">
                              {new Date(memoryBlock.created_at).toLocaleDateString()}
                            </span>
                          </div>
                          <p className="text-sm text-gray-700 line-clamp-2">
                            {memoryBlock.content?.substring(0, 200)}
                            {memoryBlock.content?.length > 200 && '...'}
                          </p>
                        </div>
