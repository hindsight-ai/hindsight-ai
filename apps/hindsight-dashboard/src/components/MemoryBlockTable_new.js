...
const MemoryBlockTable = React.forwardRef(({
  ...
  const renderCellContent = (block, columnId) => {
    switch (columnId) {
      ...
      case 'search_score':
        return (
          <div className="search-score-cell">
            {block.search_score !== undefined ? (
              <>
                <span className={`search-score-badge ${searchType || 'fulltext'}`}>
                  {(block.search_score * 100).toFixed(1)}%
                </span>
                {block.search_explanation && (
                  <div className="search-explanation" title={block.search_explanation}>
                    {truncate(block.search_explanation, 50)}
                  </div>
                )}
              </>
            ) : 'N/A'}
