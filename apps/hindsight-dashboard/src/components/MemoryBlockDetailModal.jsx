...
const MemoryBlockDetailModal = ({ blockId, isOpen, onClose }) => {
  ...
  const fetchMemoryBlockDetails = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await memoryService.getMemoryBlockById(blockId);
      setMemoryBlock(data);
      setFormData({
        errors: data.errors || '',
      ...
      setSelectedKeywords(data.keywords ? data.keywords.map(k => k.id) : []);
    } catch (err) {
  ...
  }, [blockId, isOpen, fetchMemoryBlockDetails, fetchKeywords]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await memoryService.updateMemoryBlock(blockId, {
      ...
      for (const keywordId of keywordsToAdd) {
        await memoryService.addKeywordToMemoryBlock(blockId, keywordId);
      }
      for (const keywordId of keywordsToRemove) {
        await memoryService.removeKeywordFromMemoryBlock(blockId, keywordId);
      }

      setIsEditing(false);
      fetchMemoryBlockDetails(); // Re-fetch to get the latest state
      notificationService.showSuccess('Memory block updated successfully');
    } catch (err) {
      setError('Failed to save changes: ' + err.message);
      notificationService.showError('Failed to save changes: ' + err.message);
    }
  };
  ...
  const handleDelete = async () => {
    if (window.confirm('Are you sure you want to delete this memory block?')) {
      ...
      try {
        ...
        notificationService.showError('Failed to delete memory block: ' + err.message);
      }
    }
  };
  ...
  return createPortal(
    <div 
      className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-start justify-center p-4"
      ...
      style={{
        ...
        width: '100%',
        height: '100%',
        backgroundColor: 'rgba(75, 85, 99, 0.5)', // gray-600 with 50% opacity
        zIndex: 9999
      }}
    >
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => handleInputChange('feedback_score', parseInt(e.target.value))}
                          className="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                        />
                      </div>
                        ...
                        <div className="mt-1">
                          {memoryBlock.keywords && memoryBlock.keywords.length > 0 ? (
                            <div className="flex flex-wrap gap-2">
                              {memoryBlock.keywords.map(keyword => (
