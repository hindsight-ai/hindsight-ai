...
interface AgentDetailsModalProps {
  isOpen: boolean;
  onClose: () => void;
  agent: (Agent & { description?: string; created_at?: string; updated_at?: string }) | null;
  onAgentUpdated?: () => void;
  onAgentDeleted?: () => void;
}

// Stats shape for agent related counts
interface AgentStats { memoryBlocks: number; conversations: number; loading: boolean; }

const AgentDetailsModal: React.FC<AgentDetailsModalProps> = ({ isOpen, onClose, agent, onAgentUpdated, onAgentDeleted }) => {
  const [activeTab, setActiveTab] = useState<'details'>('details');
  const [isEditing, setIsEditing] = useState<boolean>(false);
  const [editForm, setEditForm] = useState<{ agent_name: string; description: string }>({ agent_name: '', description: '' });
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [stats, setStats] = useState<AgentStats>({ memoryBlocks: 0, conversations: 0, loading: true });

  useEffect(() => {
    if (isOpen && agent) {
      setEditForm({ agent_name: agent.agent_name || '', description: agent.description || '' });
      fetchAgentStats();
    }
  }, [isOpen, agent]);

  const fetchAgentStats = async (): Promise<void> => {
    if (!agent) return;
    setStats(prev => ({ ...prev, loading: true }));
    try {
      const memoryBlocksResponse = await memoryService.getMemoryBlocks({ agent_id: agent.agent_id, per_page: 1 });
      setStats({ memoryBlocks: memoryBlocksResponse.total_items || 0, conversations: 0, loading: false });
    } catch (err) {
  ...
  const handleSaveEdit = async () => {
    if (!agent) return;
    if (!editForm.agent_name.trim()) { setError('Agent name cannot be empty'); return; }
    setLoading(true); setError(null);
    try {
      await agentService.updateAgent(agent.agent_id, { agent_name: editForm.agent_name.trim(), description: editForm.description.trim() });
      setIsEditing(false); onAgentUpdated?.();
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    if (window.confirm('Are you sure you want to delete this agent and all its associated data (memory blocks, transcripts)? This action cannot be undone.')) {
      setLoading(true);
      try { await agentService.deleteAgent(agent.agent_id); onClose(); onAgentDeleted?.(); }
      catch (err) { console.error('Failed to delete agent:', err); setError('Failed to delete agent. Please try again.'); }
      finally { setLoading(false); }
    }
  };
