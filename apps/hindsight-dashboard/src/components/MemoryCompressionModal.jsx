...
const MemoryCompactionModal = ({
  isOpen,
  onClose,
  memoryBlock,
  onCompactionApplied
}) => {
  const [compactionResult, setCompactionResult] = useState(null);
  ...
  const handleCompact = async () => {
    ...
    try {
      const result = await memoryService.compressMemoryBlock(
        memoryBlock.id,
        { user_instructions: userInstructions }
      );
      setCompactionResult(result);
    } catch (err) {
      setError(err.message || 'Failed to compact memory block');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApplyCompaction = async () => {
    ...
    try {
      ...
      if (onCompactionApplied) {
        onCompactionApplied(memoryBlock.id, compactionResult);
      }

      onClose();
    } catch (err) {
      setError(err.message || 'Failed to apply compaction');
    } finally {
      setApplying(false);
    }
  };
