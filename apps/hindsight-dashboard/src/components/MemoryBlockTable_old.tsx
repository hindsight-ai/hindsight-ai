...
// Old table retained for legacy layout / comparison during migration.
// Typed now to remove implicit any clusters.

interface SortState { field: string; order: 'asc' | 'desc'; }
interface ColumnDef { id: string; label: string; size: number; isSortable?: boolean; minSize?: number; maxSize?: number; }
// Row extends UI block with archived + search meta + feedback counts that may appear.
...
interface MemoryBlockTableOldProps {
...
const MemoryBlockTable = React.forwardRef<HTMLDivElement, MemoryBlockTableOldProps>(({
  memoryBlocks,
                ...
                onKeyDown={(e: React.KeyboardEvent<HTMLDivElement>) => {
                  if (col.id === 'select') {
                    ...
                    if (e.key === 'Enter' || e.key === ' ') {
                      e.preventDefault();
                      const checkbox = (e.currentTarget as HTMLElement).querySelector('input[type="checkbox"]') as HTMLInputElement | null;
                      checkbox?.click();
                    }
  ...
  const renderCellContent = (block: RowBlock, columnId: string): React.ReactNode => {
    switch (columnId) {
      case 'select':
        return (
          <input
      ...
      case 'archived_at':
        return (block as any).archived_at ? new Date((block as any).archived_at).toLocaleString() : 'N/A';
      case 'search_score':
        return <div className="search-score-cell">{(block as any).search_score !== undefined ? (<><span className={`search-score-badge ${searchType || 'fulltext'}`}>{(((block as any).search_score || 0) * 100).toFixed(1)}%</span>{(block as any).search_explanation && (<div className="search-explanation" title={(block as any).search_explanation}>{truncate((block as any).search_explanation, 50)}</div>)}</>) : 'N/A'}</div>;
      case 'lessons_learned':
        return (
          <>
            <span title={block.lessons_learned}>{truncate(block.lessons_learned)}</span>
            {block.lessons_learned && <CopyToClipboardButton textToCopy={block.lessons_learned} displayId={truncate(block.id,8)+':LL'} />}
          </>
        );
      case 'keywords':
        return block.keywords && block.keywords.length > 0 ? (
          <span className="keywords-list">
            {block.keywords.slice(0, 3).map((k) => (
              <span key={k.id} className="keyword-tag" onClick={() => onKeywordClick(k.keyword || k.keyword_text || '')} title={`Click to filter by ${k.keyword || k.keyword_text}`}>
                {k.keyword_text || k.keyword}
              </span>
            ))}
            {block.keywords.length > 3 && (
              <span className="more-keywords" title={block.keywords.map(k => k.keyword_text || k.keyword).join(', ')}>
      ...
      case 'errors':
        return (
          <>
            ...
            {block.errors && <CopyToClipboardButton textToCopy={block.errors} displayId={truncate(block.id,8)+':ERR'} />}
          </>
        );
      case 'agent_id':
      ...
      case 'feedback_score':
        return (
          <div className="feedback-cell">
            <span className="feedback-score-display positive" title={`Positive Feedback: ${(block as any).positive_feedback_count || 0}`}>
              👍 {(block as any).positive_feedback_count || 0}
            </span>
            <span className="feedback-score-display negative" title={`Negative Feedback: ${(block as any).negative_feedback_count || 0}`}>
              👎 {(block as any).negative_feedback_count || 0}
            </span>
            <span className="retrieval-count-display" title={`Retrieval Count: ${block.retrieval_count || 0}`}>
              📊 {block.retrieval_count || 0}
            </span>
          </div>
        );
      case 'actions':
  ...
  {memoryBlocks.map(block => renderRow(block as RowBlock))}
      </div>
    </div>
  );
});
